...
% ============== TITLE ==============
\title{\textbf{Gravitational Lensing and Light Propagation\\in a Bose-Einstein Condensate Universe}\\[0.5em]
\large Paper 2: Observable Signatures of the Acoustic Metric Framework}
...
\end{equation}
where $m$ is the boson mass. For a static background ($\mathbf{v}_0 = 0$) with relativistic sound speed ($\cs = c$), this reduces to:
\begin{equation}
...
\item \emph{Status:} \textcolor{orange}{MARGINAL} (mild tension with $m = 10^{-22}$ eV; within factor of 5).
\end{itemize}
...
\textbf{Assessment:} The value $m \approx 10^{-22}$ eV is in mild tension with Lyman-alpha forest constraints ($m > 2 \times 10^{-20}$ eV, factor of 20) but consistent with dwarf galaxy observations. This tension mirrors the broader debate in fuzzy dark matter literature and may be resolved by:
\begin{itemize}[leftmargin=*, itemsep=0.2em]
\item Improved modeling of Lyman-alpha forest systematics (thermal history, IGM physics).
\item Self-interaction effects modifying small-scale power (Paper 5 will address this).
\item Environmental dependence of effective mass in different density regimes.
\end{itemize}
...
\item \textbf{Validation Status:} The framework passes 5 of 7 validation criteria, with marginal tension on flux ratio anomalies (mild preference for higher mass $m \sim 5 \times 10^{-21}$ eV) and unclear status on the $H_0$ tension. The core value $m \approx 10^{-22}$ eV is consistent with dwarf galaxy observations but in tension with Lyman-alpha forest constraints (factor of 20).
...
\item \textbf{Resolution of Mass Constraint Tension:} Reconcile the factor-of-20 tension between dwarf galaxy constraints ($m \sim 10^{-22}$ eV) and Lyman-alpha constraints ($m > 2 \times 10^{-20}$ eV). Possible avenues: self-interactions (repulsive/attractive), environmental mass variation, improved Lyman-alpha systematics.
