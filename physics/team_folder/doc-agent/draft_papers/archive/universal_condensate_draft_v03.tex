...
% ============== TITLE ==============
\title{\textbf{The Observable Universe as a Condensate\\Expanding into a Universal Protofluid}\\[0.5em]
...
\textbf{(1) The Condensate:} A quantum BEC described by the Gross-Pitaevskii equation, with particle mass $m \sim 10^{-22}$ eV, Compton healing length $\xi = \hbar/(mc) \sim 0.064$ pc, and sound speed $c_s = c$ (relativistic limit). Gravitational Jeans instability produces solitonic cores at $\sim 1$ kpc scales. The condensate constitutes the observable universe.
...
This work lays the foundation for a seven-paper research program (Section~\ref{sec:roadmap}) addressing gravitational lensing, galaxy rotation curves, CMB power spectrum, structure formation, dark energy, gravitational waves, and quantum gravity connections.
...
\begin{postulate}[Relativistic Sound Speed]
\label{post:relativistic}
The condensate operates in the relativistic regime with sound speed $c_s = c$, where $c$ is the speed of light. This condition is achieved when the interaction energy dominates over kinetic energy: $g n_0 \gg \hbar^2 n_0^{2/3}/m$.
\end{postulate}
...
\item Postulates~\ref{post:condensate}--\ref{post:acoustic_metric} establish the physical system. Postulate~\ref{post:relativistic} ensures acoustic perturbations propagate at light speed. Postulate~\ref{post:homogeneity} enforces cosmological symmetry. Postulate~\ref{post:stiffness_matching} connects condensate mechanics to gravitational coupling.
...
With $c_s = c$ (Postulate~\ref{post:relativistic}), this is proportional to the FLRW metric:
\begin{equation}
...
\begin{itemize}[leftmargin=*, itemsep=0.1em]
\item \textbf{Observed}: Rogers \& Peiris (2021) \cite{RogersPeiris2021}: $m > 2 \times 10^{-20}$ eV at 95\% CL
\item \textbf{Predicted}: Framework assumes $m \sim 10^{-22}$ eV from rotation curve fits
\item \textbf{Discrepancy}: Factor of $\sim$200 between rotation curve preferred value and Lyman-alpha constraint. This is the most serious challenge to the framework (Paper 3, Section 6.2).
\item \textbf{Possible resolutions}: Self-interactions, environmental mass variation, Lyman-alpha systematic uncertainties
\item \textbf{Status}: \textcolor{red}{TENSION} (factor-of-200 discrepancy)
\end{itemize}
...
\textbf{(1) Lyman-Alpha Mass Tension (Test 14):} The most serious challenge. Rotation curve fits strongly prefer $m \sim 10^{-22}$ eV, while Lyman-alpha forest constraints demand $m > 2 \times 10^{-20}$ eV---a factor-of-200 discrepancy. This is not unique to the BEC cosmology framework; it affects all fuzzy/ultralight dark matter proposals. Possible resolutions include repulsive self-interactions that modify the Jeans scale at high densities, environmental mass variation between galactic and intergalactic environments, and systematic uncertainties in IGM thermal history modeling. Resolution of this tension is the single most important open problem for the framework. See Paper 3, Section 6.2 for detailed analysis.
...
\begin{table}[H]
\centering
\caption{Validation Summary: 30 Observational Tests Across All Seven Papers}
...
Galaxy Dynamics (Paper 3) & Rotation curves, mass-radius, Tully-Fisher & \textcolor{blue}{PASS (3/4)} \\
 & Lyman-$\alpha$ boson mass constraint & \textcolor{red}{TENSION (1/4)} \\
...
\midrule
\textbf{Total} & & \textbf{25 PASS, 2 PARTIAL, 1 TENSION} \\
 & & \textbf{(28/30 consistent)} \\
\bottomrule
\end{tabular}
\end{table}

The framework achieves $\sim 93\%$ validation success (28/30 tests consistent or partially consistent). The single hard tension---the Lyman-alpha boson mass constraint---is shared with all ultralight dark matter models and may be resolvable through self-interactions or systematic uncertainties. The two partial results ($H_0$ and $S_8$ tensions) show the correct qualitative behavior but require parameter optimization via full MCMC analysis with modified Boltzmann codes to determine whether quantitative resolution is achievable within the framework's parameter space.

%==============================================================================
\section{Research Roadmap: Seven-Paper Program}
\label{sec:roadmap}
%==============================================================================

This paper establishes the foundational framework. The following six papers develop specific predictions, observational tests, and theoretical extensions. All seven papers have been completed as draft manuscripts.

\subsection{Paper 1 (Current): Foundational Framework}
\label{sec:paper1}

\textbf{Title:} The Observable Universe as a Condensate Expanding into a Universal Protofluid

\textbf{Content:} Postulates, formal GP $\to$ FLRW bridge, density hierarchy, resting tension interpretation, kinematics with transient energy, complete Rankine-Hugoniot and impedance matching derivations, validation against 30 constraints, BEC parameter analysis.

\textbf{Status:} Complete (current document).
...
\textbf{Status:} Complete \cite{Paper2}. All Solar System and galaxy-scale lensing tests passed.

\subsection{Paper 3: Galaxy Rotation Curves and Solitonic Cores}
\label{sec:paper3}

\textbf{Objective:} Solve the GP equation for realistic galactic density profiles; predict rotation curves, core-cusp ratios, and scaling relations (Tully-Fisher, radial acceleration relation).

\textbf{Key Results:}
\begin{itemize}[leftmargin=*, itemsep=0.2em]
\item Solitonic core profile $\rho(r) = \rhoc/[1+0.091(r/\rc)^2]^8$ produces solid-body rotation $v \propto r$ at $r < \rc$, resolving the core-cusp problem.
\item Mass-radius relation $M_{\mathrm{sol}} \times \rc \approx 2.3 \times 10^9 M_\odot \cdot \mathrm{kpc}$ for $m = 10^{-22}$ eV.
...
\item \textbf{Critical tension}: Lyman-alpha constraint $m > 2 \times 10^{-20}$ eV conflicts with rotation curve preferred $m \sim 10^{-22}$ eV by factor of 200.
\end{itemize}

\textbf{Status:} Complete \cite{Paper3}. Core-cusp, scaling relations pass; Lyman-alpha tension identified as key open problem.
...
\textbf{Status:} Complete \cite{Paper4}. Qualitative success; quantitative resolution requires aggressive parameters and MCMC analysis.
...
\item Transient energy $\wmech(a) = -1 + \ln(a/a_c)/(3\sigma_{\mathrm{mech}}^2)$ exhibits phantom crossing consistent with DESI 2024 hints ($w_0 \approx -0.45$, $w_a \approx -1.79$).
\item Cosmic coincidence resolved through expansion dynamics rather than anthropic selection.
\end{itemize}

\textbf{Status:} Complete \cite{Paper5}. SN Ia, BAO, and $w(a)$ constraints all passed.
...
\textbf{Key Results:}
\begin{itemize}[leftmargin=*, itemsep=0.2em]
\item GW speed $|c_T/c - 1| \sim 10^{-20}$ at LIGO frequencies (for $\xi \approx 0.064$ pc), satisfying GW170817 bound ($< 10^{-15}$) by 5 orders of magnitude.
\item No birefringence: $c_T^{(+)} = c_T^{(\times)} = c$.
\item Bogoliubov dispersion negligible at LIGO/LISA frequencies ($f \gg f_\xi$); non-perturbative BEC effects at $f \lesssim f_\xi \sim 10^{-8}$ Hz (PTA band), requiring full dispersion analysis.
\item Stochastic background cutoff at $f_\xi \sim 10^{-8}$ Hz from healing length.
\item Primordial GW peak at $f_{\mathrm{peak}} \sim 10^{-5}$ Hz (LISA target).
\end{itemize}

\textbf{Status:} Complete \cite{Paper6}. All current GW observations consistent; falsifiable predictions for PTA and LISA.

\subsection{Paper 7: Quantum Gravity Connection and Planck-Scale Physics}
\label{sec:paper7}
...
\item Holographic interpretation: $\rhostiff/\rhocrit = \RH^2/3$ encodes surface-to-volume information ratio.
\item Black hole information paradox resolved through acoustic horizon analogs.
\end{itemize}

\textbf{Status:} Complete \cite{Paper7}. All Planck-scale tests passed; theoretical connections established.
...
\item \textbf{Validation}: The framework passes 25 of 30 observational tests spanning 14 orders of magnitude, with 2 partial results ($H_0$ and $S_8$ tensions---correct direction, insufficient magnitude at fiducial parameters) and 1 genuine tension (Lyman-alpha boson mass constraint). Tests cover background cosmology, gravitational lensing (Paper 2), galaxy dynamics (Paper 3), CMB perturbations (Paper 4), dark energy (Paper 5), gravitational waves (Paper 6), and Planck-scale phenomenology (Paper 7).
...
\item \textbf{Dark matter}: Identified with ultralight bosons forming a galactic-scale condensate, explaining flat rotation curves and core-soliton profiles without invoking particle dark matter.
...
The seven-paper research program (Section~\ref{sec:roadmap}), now complete in draft form, has tested the framework against gravitational lensing, galaxy dynamics, CMB perturbations, dark energy, gravitational waves, and quantum gravity. Key falsifiable predictions include:
...
The most pressing open problem is the Lyman-alpha mass tension (factor of 200 between rotation curve and forest constraints on $m$), which must be resolved through self-interactions, environmental effects, or systematic uncertainties for the framework to remain viable.
...
Requiring $c_s = c$ (Postulate~\ref{post:relativistic}):
\begin{equation}
...
\item \textbf{Dark matter} is a perturbation or localized excitation (soliton, vortex, or density fluctuation) within this background, with effective density $\rho_{\mathrm{DM}} \sim 10^{-27}$ kg/m$^3$ in galactic halos.
...
Paper 2 in this series (2026).

\bibitem{Paper3}
R.~Skavberg, ``Galaxy Rotation Curves and Solitonic Cores in a Bose-Einstein Condensate Universe,''
Paper 3 in this series (2026).

\bibitem{Paper4}
R.~Skavberg, ``CMB Power Spectrum and Perturbations in a Bose-Einstein Condensate Universe,''
Paper 4 in this series (2026).

\bibitem{Paper5}
R.~Skavberg, ``Dark Energy and Late-Time Acceleration in a Bose-Einstein Condensate Universe,''
Paper 5 in this series (2026).

\bibitem{Paper6}
R.~Skavberg, ``Gravitational Waves and Tensor Modes in a Bose-Einstein Condensate Universe,''
Paper 6 in this series (2026).

\bibitem{Paper7}
R.~Skavberg, ``Quantum Gravity Connections and Planck-Scale Physics in a Bose-Einstein Condensate Universe,''
Paper 7 in this series (2026).
