...
% ============== TITLE ==============
\title{\textbf{Planck's Field III: Galaxy Rotation Curves and Solitonic Cores}\\[0.5em]
\large Galactic Dynamics and the Core-Cusp Problem}
...
\subsubsection{Resolution via Frozen Core Dark Matter}
\label{sec:frozen_core_resolution}

Paper~1 (Section~5) introduces a definitive resolution. In the Planck field framework, the condensate has a maximum density $\rhostiff = c^2/(8\pi G) \approx 5.4 \times 10^{25}$ kg/m$^3$. Regions compressed to this density enter a pure ground state with no phonon excitations; since light is a phonon (Paper~1, Postulate~4), light cannot propagate within or escape from these \emph{frozen cores}.

Dark matter in this framework consists of frozen cores---compact objects with radius:
\begin{equation}
R_{\mathrm{fc}} = \left(\frac{6GM}{c^2}\right)^{1/3},
\end{equation}
which gives $R_{\mathrm{fc}} \ll 1$ pc for all $M < 10^{10}\,M_\odot$. Frozen cores behave as gravitationally collisionless compact objects, following Vlasov-Poisson dynamics identical to cold dark matter.

The Lyman-alpha constraint $m > 2 \times 10^{-20}$ eV was derived under the assumption that dark matter particles \emph{are} the ultralight bosons, whose quantum pressure suppresses the matter power spectrum via the FDM transfer function $T^2_{\mathrm{FDM}}(k) \to 0$ at $k > k_J$. In the Planck field:
\begin{enumerate}[leftmargin=*, itemsep=0.2em]
\item The boson mass $m \sim 10^{-22}$ eV describes the \emph{background condensate field}, not dark matter particles.
\item Dark matter (frozen cores) clusters as CDM: $P(k) \approx P_{\mathrm{CDM}}(k)$ at Lyman-alpha scales.
\item The FDM power spectrum suppression does not apply; the constraint on boson mass is not relevant.
\end{enumerate}

The rotation curve fits ($m \sim 10^{-22}$ eV) and Lyman-alpha observations ($P(k) \approx P_{\mathrm{CDM}}(k)$) are simultaneously satisfied: the former constrains the condensate field properties, the latter is consistent with frozen core dark matter.

\begin{observation}[Lyman-Alpha Tension Resolved]
The apparent factor-of-200 tension between rotation curve fits ($m \sim 10^{-22}$ eV) and Lyman-alpha constraints ($m > 2 \times 10^{-20}$ eV) is resolved by the frozen core dark matter interpretation (Paper~1, Section~5). The Lyman-alpha constraint assumes dark matter \emph{is} the ultralight boson (FDM). In the Planck field, dark matter consists of frozen cores---compact objects at density $\rhostiff$---which cluster as CDM. The boson mass $m \sim 10^{-22}$ eV describes the background field, not dark matter particles, and the FDM constraint does not apply.
\end{observation}
...
\item \textbf{Lyman-alpha forest:} The frozen core dark matter interpretation predicts $P(k) \approx P_{\mathrm{CDM}}(k)$ at Lyman-alpha scales, with no FDM-like suppression. If future observations detect suppression consistent with FDM at $m \sim 10^{-22}$ eV, this would require revisiting the frozen core interpretation. \textbf{Test:} DESI/4MOST high-resolution Lyman-alpha spectra at $z \sim 2$--$4$.
\end{enumerate}
...
\item The matter power spectrum at Lyman-alpha scales ($k \sim 1$--$10$ Mpc$^{-1}$) shows wave-like suppression consistent with FDM at $m \sim 10^{-22}$ eV, contradicting the CDM-like clustering predicted by frozen core dark matter.
...
\item \textbf{Lyman-Alpha Resolution:} The apparent tension between rotation curve fits ($m \sim 10^{-22}$ eV) and Lyman-alpha constraints ($m > 2 \times 10^{-20}$ eV) is resolved by the frozen core dark matter interpretation (Paper~1, Section~5). Dark matter consists of frozen cores at density $\rhostiff$, which cluster as CDM. The boson mass describes the background field, not dark matter particles.
...
Paper 3 establishes that the BEC framework successfully reproduces galactic dynamics observations while making specific falsifiable predictions distinct from CDM. The previously identified Lyman-alpha tension is resolved by the frozen core dark matter interpretation from Paper~1.
...
\item \textbf{Frozen Core Mass Spectrum:} Determine the mass distribution $N(M)$ of frozen cores through numerical simulation of condensation dynamics. For $M > 3.8\,M_{\mathrm{Jupiter}}$, gravitational self-compression is sufficient; for smaller masses, primordial formation mechanisms during the condensation phase transition must be modeled.
...
The frozen core dark matter interpretation (Paper~1, Section~5) resolves the previously identified Lyman-alpha mass tension. Dark matter consists of compact frozen cores at density $\rhostiff$, behaving as CDM on all astrophysically relevant scales. The boson mass $m \sim 10^{-22}$ eV characterizes the condensate field properties, not the dark matter clustering scale. The remaining open question is the frozen core mass spectrum $N(M)$, to be determined by numerical simulations of condensation dynamics.
